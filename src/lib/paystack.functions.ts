...
// Create an admin client to bypass RLS safely on the backend server
...
export const initPayment = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator(
    (data: { propertyId: string; plan: PaymentPlan; email: string; nights?: number }) => data,
  )
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;

    // Fetch property (RLS safely applies as buyer using the context client)
    const { data: property, error: pErr } = await supabase
      .from("properties")
      .select("id, title, price, listing_type, agent_id")
      .eq("id", data.propertyId)
    ...
    // Bypass RLS here to record the payment securely via the Admin client
...
export const verifyPayment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const status: string = json?.data?.status ?? "failed";
    const dbStatus =
      status === "success" ? "success" : status === "abandoned" ? "abandoned" : "failed";

    const { userId } = context;

    // Bypass RLS on updates during verification to prevent failures if RLS limits UPDATE permissions
    const supabaseAdmin = getSupabaseAdmin();
